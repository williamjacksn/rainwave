...
help_classes: dict[
    str,
    api.web.APIHandler
    | api.web.RainwaveHandler
    | api.web.PrettyPrintAPIHandler
    | api.web.HTMLRequest,
] = {}
url_properties = (
...
def sectionize_requests():
    for url, handler in help_classes.items():
        ...
        elif isinstance(handler, api.web.PrettyPrintAPIHandler):
...
class IndexRequest(api.web.HTMLRequest):
    ...
    def write_class_properties(self, url, handler):
        ...
        if (
            ...
            or isinstance(handler, api.web.PrettyPrintAPIHandler)
        ) and url.find("(") == -1:
